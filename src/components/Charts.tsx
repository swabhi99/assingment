...
useEffect(() => {
  ...
  const myChart = new Chart(document.getElementById('myChart')  as HTMLCanvasElement , chartConfig);
  return () => {
    myChart.destroy();
  };
}, [chartData]);
  ...
  return (
    <div>
        <Heading >Charts and Maps</Heading>
        ...
        {window.innerWidth>900?<Box padding={"10px"} w={"19%"} boxShadow={"rgba(0, 0, 0, 0.24) 0px 3px 8px"}>
               ...
               <br />
               <br />
               <Box><Link style={{textDecoration:"none",fontSize:"20px",fontWeight:"bold"}} to="/charts">Charts & Maps</Link></Box>
